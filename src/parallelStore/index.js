...
export const ParallelContext = createContext(initialState);

export default function ParallelStore({ children }) {
  const [state, dispatch] = useReducer(Reducer, initialState);
  const globalCommit = (context, payload) => commit(state, context, payload);
  return (
    <ParallelContext.Provider value={{ state, dispatch, commit: globalCommit }}>
      {children}
